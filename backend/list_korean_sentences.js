...
const getKoreanWordsByLesson = async (lessonId) => {
  try {
    ...
    const koreanSentences = vocabularies.map((vocab) => vocab.example_sentence_korean);

    // Log the results
    console.log('Korean Sentences:', koreanSentences);
    
  } catch (error) {
    console.error('Error retrieving Korean sentences:', error);
  } finally {
